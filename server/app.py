...
class IndividualCRUD(Resource):
    def get(self, individual_id):
        ...
        return jsonify(get_individual(individual_id))

    def put(self, individual_id):
        """
        Updates the information of the salamander with the given id.
        """
        body = request.json
        individual = get_individual(individual_id)
        if "name" in body:
            individual.name = body["name"]
        db.session.commit()
        return individual
...
class SightingCRUD(Resource):
    ...
    def put(self, sighting_id):
        """
        Updates the sighting with the given id.
        """
        body = request.json
        sighting = get_sighting(sighting_id)
        if "individual_id" in body:
            sighting.individual_id = body["individual_id"]
        if "location_id" in body:
            sighting.location_id = body["location_id"]
        if "date" in body:
            sighting.date = datetime.strptime(body["date"], "%Y-%m-%dT%H:%M:%S.%f")
        db.session.commit()
        return sighting
